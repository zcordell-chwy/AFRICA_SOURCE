            ...
            if ($yadis) {
                ...
                foreach ($headers as $header) {
                    if (preg_match('#X-XRDS-Location\s*:\s*(.*)#', $header, $m)) {
                        $headerValue = trim($m[1]);
                        // Allow customers to override the default built URL based on the XRDS Location header
                        $hookData = array('url' => $url, 'headerValue' => $headerValue, 'builtUrl' => $this->build_url(parse_url($url), parse_url($headerValue)));
                        \RightNow\Libraries\Hooks::callHook('openlogin_build_url', $hookData);
                        $url = $hookData['builtUrl'];
                        $next = true;
                    }
