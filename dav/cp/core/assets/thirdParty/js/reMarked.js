...
reMarked = function(opts) {
	...
	this.render = function(ctr) {
		...
		if (cfg.link_list) {
			...
			for (var k in links) {
				...
				re += "  [" + (+k+1) + "]: " + (links[k].e.href || links[k].e.src) + title + "\n";
			}
		}
	...
	lib.tag = klass({
		...
		initK: function()
		{
			...
			if (this.e.hasChildNodes()) {
				...
				var inlRe = /^(?:a|strong|code|em|sub|sup|del|i|u|b|big|center)$/,
				 	childNodes = this.e.childNodes,
					n, name;
				for (i in childNodes) {
					...
					if (name == "txt" && /^\s+$/.test(n.textContent)) {
						...
						if (i == 0 || i == childNodes.length - 1 || !this.p)
							continue;
	...
	lib.inl = lib.tag.extend({
		...
		lib.txt = lib.inl.extend({
			initK: function()
			{
				var textProperty = ('textContent' in this.e) ? 'textContent' : (('innerText' in this.e) ? 'innerText' : 'nodeValue');
				this.c = escape(this.e[textProperty]).split(/^/gm);
			},
			rendK: function()
			{
