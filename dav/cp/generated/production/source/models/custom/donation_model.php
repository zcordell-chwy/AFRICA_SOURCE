...
class donation_model  extends \RightNow\Models\Base
{
    ...
    public function createDonationAfterTransaction($amt, $c_id, array $items, $trans_id, $paymentMethod)
    {
        ...
        $itemDescs = array();
        if (count($items) < 1) {
            helplog(__FILE__, __FUNCTION__ . __LINE__, "", "No items passed to method");
            return false;
        }
        ...
        foreach ($items as $index => $item) {
            ...
            helplog(__FILE__, __FUNCTION__ . __LINE__, print_r($item, true), "");
            helplog(__FILE__, __FUNCTION__ . __LINE__, isset($item) ? json_encode($item) : "", "");
            switch ($item['type']) {
                case DONATION_TYPE_PLEDGE:
                    logMessage("Donation Model > DONATION_TYPE_PLEDGE");
                    helplog(__FILE__, __FUNCTION__ . __LINE__, "", "Donation Model > DONATION_TYPE_PLEDGE");
    ...
    {
        helplog(__FILE__, __FUNCTION__ . __LINE__, "Recieved Data:" . $amt." ".$c_id, "");
    ...
    public function createDonationAfterTransaction_2($amt, $c_id, array $items, $trans_id, $paymentMethod, $f_id)
    {
        ...
        logMessage("Donation Model > DONATION_TYPE_SPONSOR");
        logMessage($donationId, !empty($this->CI->session->getSessionData('monthly_amt')) ? $this->CI->session->getSessionData('monthly_amt') : '', !empty($this->CI->session->getSessionData('oneTime_amt')) ? $this->CI->session->getSessionData('oneTime_amt') : '', $c_id, $f_id, !empty($this->CI->session->getSessionData('appeal')) ? $this->CI->session->getSessionData('appeal') : '', "", $paymentMethod, null, "0");
    ...
    public function addPledgeToDonation($donationId, $mon = 0, $one = 0, $c_id, $fund = null, $appeal = null, $childId = null, $paymentMethod, $pledgeId = null, $isWomensScholarship = false)
    {
        ...
        try {
            ...
            if ($mon != 0) {
                ...
                if (is_null($fund) || $fund < 1) {
                    if (is_null($childId)) {
                        ...
                        helplog(__FILE__, __FUNCTION__ . __LINE__, "", "Invalid Fund");
                        return false;
                    }
                } else {
                ...
                if ($isWomensScholarship) {
                    ...
                    $descr = $pledge->Woman->WomanRef . " " . $pledge->Woman->GivenName;
                }
            ...
            if ($one != 0) {
                ...
                if (!$pledgeId) { //if this ia pledge payment then we'll associate the existing pledge to the donation/transaction
                    ...
                    if ($childId != null) {
                        if ($isWomensScholarship) {
                            ...
                            $pledge->Appeals = intval(WEB_APPEAL_ID); //WEB
                            $pledge->pledgefor = RNCPHP\donation\pledgefor::fetch(2);
                            $pledge->Descr  = $pledge->Woman->WomanRef . " " . $pledge->Woman->GivenName;
                        } else {
                            $pledge->Child = intval($childId);
                            $pledge->ChildSponsorship = true;
                            //need to set teh sponsored child as "sponsored"
                    ...
                    helplog(__FILE__, __FUNCTION__ . __LINE__, ": pledge object" . print_r($pledge->ID, true), "");
                } else {
                ...
                helplog(__FILE__, __FUNCTION__ . __LINE__, ": paymethod ID:" . $paymentMethod->ID, "");
                helplog(__FILE__, __FUNCTION__ . __LINE__, ": donation ID:" . $donation->ID, "");
                helplog(__FILE__, __FUNCTION__ . __LINE__, ": pledge  ID:" . $pledge->ID, "");
                helplog(__FILE__, __FUNCTION__ . __LINE__, ": donation2pledge object", "");
                helplog(__FILE__, __FUNCTION__ . __LINE__, print_r($donation2Pledge, true), "");
        ...
        $id = $pledge->ID;
        helplog(__FILE__, __FUNCTION__ . __LINE__, "donation item id: $id", "");
        return $id;
    }
