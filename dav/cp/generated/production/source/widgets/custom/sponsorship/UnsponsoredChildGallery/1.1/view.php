<div id="rn_<?=$this -> instanceID; ?>" class="<?= $this -> classList ?> clearfix">
	...
	<!-- Don't want filters on womens page ZC 5/18/22 -->
     	...
     	<? endif; ?>
			...
			<?if($this->data['attrs']['data_source'] == 'woman'): ?>
				<label>Program Type</label>
				 <select class="rn_ProgramFilter">
					<?php 
						$selectedValue = isset($this->data['unsponsoredChildren']['metadata']['filters']['program']) ? $this->data['unsponsoredChildren']['metadata']['filters']['program'] : 0; 
						$options = array('0' => 'All', '1' => 'Dream Beauty Academy', '2' => 'Vocational Sewing');
	...
	<div class="rn_UnsponsoredChildImageGalleryContainer">
		...
		<div class="rn_UnsponsoredChildImageGallery">
			...
			<?php if($numMatches > 0): ?>
				...
				<div class="rn_UnsponsoredChildImageLinkContainer">
				   ...
				   <!--build different link based on data source-->
				   ...
				   <?if($this->data['attrs']['data_source'] == 'woman'): ?>
						<a id="rn_UnsponsoredChildImageLink<?= $child -> ID ?>" class="rn_UnsponsoredChildImageLink" href="javascript: void(0);"
							...
							data-ref="<?= $child -> WomanRef ?>"
							data-program="<?= $child -> Program ?>"
							data-remaining="<?= $child -> ActivePledgeTotal ?>"
							data-bio="<?= $child -> FamilySituation ?>"
		...
		<div class="rn_UnsponsoredChildImageGalleryPaginator">
			...
			if($this->data['attrs']['data_source'] == 'woman'){
				$paginatorLinkURL = '/app/womens_ministry';
			}else if ($this -> data['attrs']['advocacy_page']) {
