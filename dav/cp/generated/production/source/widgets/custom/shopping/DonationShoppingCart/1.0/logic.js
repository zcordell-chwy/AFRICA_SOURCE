...
Custom.Widgets.shopping.DonationShoppingCart =
  Custom.Widgets.shopping.ShoppingCart.extend({
    ...
    overrides: {
      ...
      renderLineItem: function (lineItem) {
        ...
        lineItemNode
          .one("input.rn_LineItemOneTimeAmount")
          .setAttribute("data-prev", lineItem.customData.amountOneTime);
        // Register change event handler for one-time amount input
        lineItemNode
          .one("input.rn_LineItemOneTimeAmount")
          .on("change", this.onDonationAmountChange, this);
        ...
        if (lineItem.customData.donationType === "fund") {
          ...
          // calculate a delta value in the 'onDonationAmountChange' routine
          lineItemNode
            .one("input.rn_LineItemMonthlyAmount")
            .setAttribute("data-prev", lineItem.customData.amountMonthly);
    ...
    onDonationAmountChange: function (evt) {
      var amountInput = evt.currentTarget,
        ...
        isOneTimeAmount = amountContainer.hasClass(
          "rn_LineItemOneTimeAmountContainer"
        ),
    ...
    updateDonationAmount: function (
      ...
      // Update price in UI
      this.updateLineItemPriceInShoppingCartUI(
        id,
        lineItem.merch.price + (newAmount - oldAmount)
      );
      // Update total in UI
      this.updateTotal(newAmount - oldAmount, 0);
      // Call AJAX method to update custom data and price in session
      this.updateLineItemCustomDataInShoppingCartSessionDataAJAX(
        id,
        updatedCustomData,
        function () {
        ...
        function () {
          ...
          // Restore old amount
          this.updateLineItemAmountInShoppingCartUI(
            id,
            oldAmount,
            oneTimeOrMonthly
          );
     ...
     */
    updateLineItemAmountInShoppingCartUI: function (
      id,
      updatedAmount,
      oneTimeOrMonthly
    ) {
