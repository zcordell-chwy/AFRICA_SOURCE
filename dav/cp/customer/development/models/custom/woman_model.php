...
class woman_model extends \RightNow\Models\Base
{
    ...
    private function buildWomanObj(RNCP\sponsorship\Woman $woman)
    {
        ...
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, 0, 0, __LINE__, print_r($woman, true), "CartStorage");
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, 0, 0, __LINE__, print_r($woman->Program, true), "CartStorage");
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, 0, 0, __LINE__, print_r($woman->Program->LookupName, true), "CartStorage");
    ...
    public function getWoman($id)
    {
        try {
            ...
            $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, 0, 0, __LINE__, "woman query: select SPONSORSHIP.Woman from SPONSORSHIP.Woman WHERE ID = " . $id, "CartStorage");
            while ($woman = $resultSet->next()) {
                $women[] = $this->buildWomanObj($woman);
            }
            return $women;
        } catch (Exception $e) {
            $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, 0, 0, __LINE__, print_r($e->getMessage(), true), "CartStorage");
        } catch (RNCPHP\ConnectAPIError $err) {
            $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, 0, 0, __LINE__,  print_r($err->getMessage(), true), "CartStorage");
        }
    }
