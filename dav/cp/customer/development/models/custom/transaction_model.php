...
class transaction_model extends \RightNow\Models\Base
{
    ...
    public function startProcessingTransaction($t_id, $transactionType = NULL)
    {
        ...
        if (!$trans instanceof RNCPHP\financial\transactions) {
            // helplog(38, __FUNCTION__ . ": No Transaction found");
        ...
        if (is_null($status) || $status < 1) {
            // helplog(45, __FUNCTION__ . " Could not determin status");
        ...
        switch ($transactionType) {
            case FS_SALE_TYPE:
                if (!$this->isAllowedStatus($status, $this->allowedChargeStatus)) {
                    // helplog(53, __FUNCTION__ . ": Invalid transaction status: " . $status);
            ...
            case FS_REFUND_TYPE:
                if (!$this->isAllowedStatus($status, $this->allowedRefundStatus)) {
                    // helplog(59, __FUNCTION__ . ": Invalid transaction status: " . $status);
                    helplog(__FILE__, __FUNCTION__ . __LINE__, ": Invalid transaction status: " . $status, "");
                    return false;
                }
                break;
            case FS_REVERSAL_TYPE:
                if (!$this->isAllowedStatus($status, $this->allowedReversalStatus)) {
                    // helplog(65, __FUNCTION__ . ": Invalid transaction status: " . $status);
                    helplog(__FILE__, __FUNCTION__ . __LINE__, ": Invalid transaction status: " . $status, "");
                    return false;
                }
                break;
        ...
        try {
            // helplog(76, __FUNCTION__ . ": Setting Transaction status to " . TRANSACTION_PROCESSING_STATUS_ID);
            ...
            // helplog(83, __FUNCTION__ . ": " . $e->getMessage());
            helplog(__FILE__, __FUNCTION__ . __LINE__, "", $e->getMessage());
            return false;
        }
        // helplog(86, __FUNCTION__ . ": Returning True");
    ...
    public function updateTransStatus($tran_id, $statusID = null, $paymentMethodId = null, $PNRef = null)
    {
        ...
        if (!$trans instanceof RNCPHP\financial\transactions) {
            // helplog(252, " invalid type");
            helplog(__FILE__, __FUNCTION__ . __LINE__, " invalid type", "");
            return false;
        }
        if (!is_null($paymentMethodId) && $paymentMethodId > 0) {
            // helplog(256, "valid paymethod id = $paymentMethodId");
        ...
        try {
            ...
            // helplog(275, "Updating Transaction: statusID = $statusID and TRANSACTION_SALE_SUCCESS_STATUS_ID = " . TRANSACTION_SALE_SUCCESS_STATUS_ID);
            ...
            // helplog(284, ": " . $e->getMessage());
            helplog(__FILE__, __FUNCTION__ . __LINE__, "", $e->getMessage());
            return false;
        } catch (RNCPHP\ConnectAPIError $e) {
            helplog(__FILE__, __FUNCTION__ . __LINE__, "", $e->getMessage());
        }
        // helplog(289, ":transaction status updated:" . $statusString);
