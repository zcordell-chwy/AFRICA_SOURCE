...
class sponsorship_model extends \RightNow\Models\Base {
    ...
    public function isChildSponsored($id){
        $this->CI->logging->logFunctionCall(self::$CLASS_SCOPE, 'isChildSponsored', 
            array('$id' => $id), $this->CLASS_LOG_LEVEL);
        ...
        try{
            ...
            if($child->SponsorshipStatus->ID === 3){
                $this->CI->logging->logMsg('Child has a SponsorshipStatus flag of 3, indicating he/she is sponsored.', 
                    $this->CI->Logging->LOG_LEVEL_DEBUG_FULL, $this->CLASS_LOG_LEVEL);
                $status->isSponsored = true;
            }else{
                $this->CI->logging->logMsg('Child DOES NOT have a SponsorshipStatus flag of 3, indicating he/she is NOT sponsored.',
                    $this->CI->Logging->LOG_LEVEL_DEBUG_FULL, $this->CLASS_LOG_LEVEL);
            }
        }catch(\Exception $e){
    ...
    public function isChildRecordLocked($id){
        $this->CI->logging->logFunctionCall(self::$CLASS_SCOPE, 'isChildRecordLocked', 
            array('$id' => $id), $this->CLASS_LOG_LEVEL);
    ...
    public function lockChildRecord($id){
        $this->CI->logging->logFunctionCall(self::$CLASS_SCOPE, 'lockChildRecord', 
            array('$id' => $id), $this->CLASS_LOG_LEVEL);
            ...
            try{
                ...
                while($res = $resultSet->next()){
                    ...
                    try{
                        ...
                        if( strtoupper($child->ChildRef) !== 'NOCHILD' &&
