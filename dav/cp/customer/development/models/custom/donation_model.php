...
class donation_model  extends \RightNow\Models\Base
{
    ...
    public function createDonationAfterTransaction($amt, $c_id, array $items, $trans_id, $paymentMethod)
    {
        ...
        $itemDescs = array();
        if (count($items) < 1) {
            $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "No items passed to method", "Pledge");
            return false;
        }
        ...
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "Saved Donation:" . $donationId, "Pledge");

        if ($donationId < 1) {
            $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "No donation created", "Pledge");
            return false;
        }
    ...
    public function savedonation($amt, $c_id, $items)
    {
        try {
            ...
            $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, $e->getMessage(), "Pledge");
            return 0;
        }
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "New Donation Created with Contact:" . $newDonation->Contact->ID . " DonationDate:" . $newDonation->DonationDate . " Amount:" . $newDonation->Amount . " Type:" . $newDonation->Type->LookupName, "Pledge");
        return $id;
    }
    ...
    {
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "Adding pledge to donation", "Pledge");
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "Donation: $donationId, PayMethod: $paymentMethod->ID, Monthly: $mon, One Time: $one, Contact: $c_id, Fund: $fund, Appeal: $appeal, ChildId: $childId", "Pledge");
        ...
        try {
            ...
            if ($one != 0) {
                ...
                $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "paymethod ID:" . $paymentMethod->ID, "Pledge");
                $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "donation ID:" . $donation->ID, "Pledge");
                $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "pledge  ID:" . $pledge->ID, "Pledge");
                $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "donation2pledge object", "Pledge");
                $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, print_r($donation2Pledge, true), "Pledge");
        ...
        $id = $pledge->ID;
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, $c_id, 0, __LINE__, "donation item id: $id", "Pledge");
        return $id;
    }

    public function savePayMethodToPledge($pledgeId, $paymentMethod)
    {
        $this->CI->model('custom/log_model')->log(__FILE__, __FUNCTION__, 0, 0, __LINE__, "No donation created", "Pledge");
        $pledge = RNCPHP\donation\pledge::fetch($pledgeId);
