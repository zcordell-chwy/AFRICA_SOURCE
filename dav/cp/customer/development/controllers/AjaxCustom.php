...
class AjaxCustom extends \RightNow\Controllers\Base
{
    ...
    function runStoredPayment()
    {
        ...
        if (is_numeric(intval($this->CI->session->getSessionData('total'))) < intval(RNCPHP\Configuration::fetch('CUSTOM_CFG_MINMUM_AMNT'))) { //5
    ...
    {
        try {
            $response = $this->model('custom/user_tracking')->checkIsUserQualified();
            print_r($response);
            // $count = 0;
            ...
            //     print_r("USER TRACKING OBJ: IP = " . $ut->user_ip . ", TIME: " . date("Y-m-d h:i:sa", $ut->form_submit_time));
