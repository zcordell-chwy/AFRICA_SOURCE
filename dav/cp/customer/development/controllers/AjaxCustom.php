...
class AjaxCustom extends \RightNow\Controllers\Base
{
    ...
    function deletePayMethods()
    {
        ...
        echo json_encode($this->createResponseObject("Success!", array(), "/app/paymentmethods/"));
    }
    ...
    function deletePayMethod()
    {
        ...
        echo json_encode($this->createResponseObject($success, array(), "/app/account/transactions/c_id/" . $this->session->getProfileData('contactID') . "/action/deleteConfirm/", array('confirmMessage' => getMessage(CUSTOM_MSG_DELETE_PAYMETHOD))));
    }
    ...
    function cancelPledge()
    {
        ...
        echo json_encode($this->createResponseObject("Success!", array(), "/app/pledge_confirm/pledge_id/" . $pledge_id . "/"));
    }
    ...
    function updatePledge()
    {
        ...
        if ($cleanFormArray['pledge_id'] > 0) {
            try {
                ...
                echo json_encode($this->createResponseObject("Error", "Error updating pledge " . $e->getMessage()));
            }
        }

        echo json_encode($this->createResponseObject("Success!", array(), "/app/pledge_confirm/pledge_id/" . $cleanFormArray['pledge_id'] . "/"));
    ...
    function runStoredPayment()
    {
        ...
        if (count($sanityCheckMsgs) > 0) {
            echo json_encode($this->createResponseObject("Invalid Input", $sanityCheckMsgs));
            return;
        }
        ...
        if ($frontstreamResp['isSuccess'] === true) {
            ...
            if ($donationId === false || $donationId < 1) {
                echo json_encode($this->createResponseObject("The payment processed correctly, but your donation may not have been properly credited.  Please contact donor services", $sanityCheckMsgs));
                return;
            }
            ...
            echo json_encode($this->createResponseObject("Success!", array(), $this->redirectCcChargeSuccessLocation . "t_id/" . $transactionId . "/authCode/" . $this->model('custom/frontstream_model')->authCode . "/"));
            return;
        }
        logMessage("sending NOT success response");
        echo json_encode($this->createResponseObject("Error Processing Payment", $this->model('custom/frontstream_model')->getEndUserErrorMsg()));
        return;
    }
    ...
    function changePayMethod()
    {
        ...
        echo json_encode($this->createResponseObject($success, array(), "/app/account/pledges/c_id/" . $this->session->getProfileData('contactID') . "/action/updateConfirm/p_id/" . $cleanFormArray['pledgeId'] . "/", null));
    }
    ...
    function createAdvocacyRelationship()
    {
        ...
        echo json_encode($this->createResponseObject('Advocacy relationship created', array(), "/app/advocacy/event/" . $rawFormDataArr->items[0]->eventId, $advocacyNotes));
    }
    ...
    function getCartData()
    {
        ...
        echo json_encode($this->createResponseObject('Session Cart Data', array(), null, array(
            'total' => $total,
            'items' => $items,
    ...
    private function createResponseObject($message, array $errors, $redirectLocation = null, $includeObject = null)
    {
        ...
        return $result; //return json_encode((object)$result);
    }
    ...
    function sendStatement()
    {
        ...
        if ($contact->ID > 0) {
            ...
            if ($giftReceiptMailSend != 1) {
                ...
                echo json_encode($this->createResponseObject("Mail Not Sent", $errorMsgs));
            } else {
                echo json_encode($this->createResponseObject("1", array()));
            }
        } else {
            $errorMsgs[] = "Contact ID is not valid";
            echo json_encode($this->createResponseObject("Not successful", $errorMsgs));
        }
    }
