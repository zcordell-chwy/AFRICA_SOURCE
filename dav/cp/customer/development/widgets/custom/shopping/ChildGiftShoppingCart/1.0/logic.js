...
Custom.Widgets.shopping.ChildGiftShoppingCart = Custom.Widgets.shopping.ShoppingCart.extend({ 
    /**
    ...
    overrides: {
        ...
        renderLineItem: function(lineItem){
            ...
            if(this.renderedChildGroupContainers[lineItem.customData.childID] === undefined){
                // Build child line item group container
                childGroupContainer = this.buildChildGroupContainer(lineItem.customData);
                this.renderedChildGroupContainers[lineItem.customData.childID] = childGroupContainer;
                childGroupContainer.setAttribute("data-line-item-count", 0);
            }else{
                childGroupContainer = this.renderedChildGroupContainers[lineItem.customData.childID];
            }
            ...
            if(parseInt(childGroupContainer.getAttribute("data-line-item-count")) === 0){
                shoppingCartLineItemContainer.append(childGroupContainer);
            }
            childGroupContainer.one(".rn_ChildLineItemsContainer").append(lineItemNode);
        ...
        */
        removeLineItemFromShoppingCartUI: function(id){
            var lineItemNode = this.Y.one(this.baseSelector + " .rn_ShoppingCartLineItemContainer div.rn_LineItem[data-id=\"" + id + "\"]"),
                ...
                groupLineItemCount = parseInt(lineItemGroupNode.getAttribute("data-line-item-count"));
            lineItemNode.remove(true);
            if(--groupLineItemCount === 0){
                lineItemGroupNode.remove();
            }
            lineItemGroupNode.setAttribute("data-line-item-count", groupLineItemCount);
        }
    },
    ...
    */
    buildChildGroupContainer: function(lineItemCustomData){
        var childLineItemGroupHTML = new EJS({text: this.getStatic().templates.childLineItemGroup}).render({
