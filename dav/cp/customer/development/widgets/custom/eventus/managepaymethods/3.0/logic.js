...
Custom.Widgets.eventus.managepaymethods = RightNow.Widgets.extend({
    ...
    onAddNewPayLinkClicked: function () {
        this.newPaymentFormDialog || (this.newPaymentFormDialog = this.createDialog());

        this.newPaymentFormDialog.show();

        RightNow.UI.Dialog.enableDialogControls(this.newPaymentFormDialog, this.dialogKeyListener);

        this.disabled = false;
    },

    createDialog: function () {
        var newPaymentFormDialog = RightNow.UI.Dialog.actionDialog(this.data.attrs.title,
            this.newPaymentForm,
            {
                buttons: [
                    {
        ...
        this.dialogKeyListener = RightNow.UI.Dialog.addDialogEnterKeyListener(newPaymentFormDialog, this.savePaymentMethod, this);
    ...
    validatePaymentMethod: function () {
        ...
        var data = Object.fromEntries((new FormData(document.getElementById("newPaymentForm"))).entries());
        var validated;
        ...
        switch (data.paymenttype) {
            case "card":
                ...
                if (!validated) {
                    if (!(/^\d+$/.test(data.cardnumber.split(" ").join("")))) {
                        validated = "Please input a properly formated credit card: 1111222233334444 / 1111 2222 3333 4444.";
                    }
                } else {
            ...
            case "check":
                ...
                if (!validated) {
                    if (!(/^\d+$/.test(data.accountnumber)) || data.accountnumber.length > 17) {
                        validated = "Input a valid account number.";
                    } else if (!(/^\d+$/.test(data.routingnumber)) || data.routingnumber.length != 9) {
    ...
    validate: function (array, offset = 0) {
        var allFilled = array[offset] === null ||
        ...
        return (offset + 1 < array.length) ? allFilled || this.validate(array, offset + 1) : allFilled;
    },
    ...
    saveCardPaymentMethod: function (data) {
        ...
        RightNow.Ajax.makeRequest(this.data.attrs.save_card_payment_method, eventObj.data, {
    ...
    saveCheckPaymentMethod: function (data) {
        ...
        RightNow.Ajax.makeRequest(this.data.attrs.save_check_payment_method, eventObj.data, {
            successHandler: this.ajaxCallback,
            scope: this,
            data: eventObj,
            json: false
    ...
    successCallback: function(response, originalEventObj) {
        ...
        return Function('response', 'originalEventObj', this.data.attrs.success_callback)(response, originalEventObj);
    },

    closeDialog: function () {
        this.disabled = true;
        RightNow.UI.Dialog.disableDialogControls(this.newPaymentFormDialog, this.dialogKeyListener);
