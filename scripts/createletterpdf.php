...
function createPDF($incID){
    ...
    try{
        ...
        $pdf->StartPageGroup();
        $pdf -> AddPage();
        $pdf -> Image('http://africanewlife--tst.custhelp.com/euf/assets/images/ANLM_Logo_Black.jpg', 10, 6, 75, 25);
        $pdf->Ln(30);
...
function _logToFile( $message){
    ...
    $fp = fopen('/tmp/letterLogs_'.date("Ymd").'.log', 'a');
    fwrite($fp,  "donation_model @ $lineNum : ".$message."\n");
    fclose($fp);
