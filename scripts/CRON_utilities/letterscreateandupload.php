...
if(!is_dir('/tmp/letterupload/'.date('Y-m-d'))){
    ...
    $roql = "Select Incident from Incident where Incident.Category.ID = " . INC_CAT_ONLINE_LETTER .
            " and Incident.StatusWithType.Status.ID = " . INC_STATUS_NEW . " AND Incident.CustomFields.CO.ChildRef NOT NULL ORDER BY Incident.CustomFields.CO.ChildRef.LookupName ASC, Incident.ID Limit 25";
