...
if (!is_dir('/tmp/logfiles')){
    $oldumask = umask(0);
    mkdir('/tmp/logfiles', 0775, true);
    umask($oldumask);
}

$logFile = fopen("/tmp/logfiles/dailylog_".date("Y_m_d").".log", "a");
fwrite($logFile, date("Y-m-d H:i:s  "). "IP:".$ip."\n");
fclose($logFile);
...
switch ($hour) {
	...
	case 3://3am PST
        ...
        case 6: //6am PST
            ...
            break;
        case 7: //7am PST
            //require_once("update_card_info.php");
            break;
        case 14:
            //require_once("womanupdate.php");
	   break;
