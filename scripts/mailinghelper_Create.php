...
implements RNCPM\ObjectEventHandler {
        ...
        try {
            ...
            if ($pledges) {
                foreach ($pledges as $pledge) {
                    ...
                    if ($pledge -> Child && $pledge -> SentSponsorEmail != 1 && !$pledge->copiedFromPledge) {
                        ...
                        static::$updatedNote[] =  "\nsponsorship mail send = " . $sponsorshipMailSend;
            ...
            if ($trans -> donation -> Contact && $skipReciept == false) {
                ...
                if($trans->currentStatus->ID == static::$refundedStatus){// refund receipt
                    ...
                    if ($refundMailSend != 1) {
                        static::$updatedNote[] =  "\nCouldn't send refund mailing." ;
                    } else {
                        static::$updatedNote[] =  "\nSent refund mailing to Contact ".$trans -> donation -> Contact->ID;
                    }
                    ...
                    if ($giftReceiptMailSend != 1) {
                        static::$updatedNote[] =  "\nCouldn't send gift receipt mailing." ;
                    } else {
                        static::$updatedNote[] =  "\nSent gift receipt mailing with id: " . static::$giftrecieptMailingID . " to Contact ".$trans -> donation -> Contact->ID;
                    }
                    ...
                    if ($receiptMailSend != 1) {
                        static::$updatedNote[] =  "\nCouldn't send receipt mailing." ;
                    } else {
                        static::$updatedNote[] =  "\nSent mailing with id: " . static::$receiptMailingID . " to Contact ".$trans -> donation -> Contact->ID;
                    }
                }
    ...
    public static function getPledges($donationId = 0) {
        ...
        try {
            $roql = sprintf("SELECT financial.internalTransaction from financial.internalTransaction as intTrans where intTrans.transactionRef.donation.ID = '%s' order by intTrans.pledge.ID DESC", $donationId);
            ...
            foreach($pledges as $pl){
                ...
                static::$totalsTable .= "<tr><td>".date('m/d/Y')."</td><td>".$childRef."</td><td>".$pl->Fund->Descriptions[0]->LabelText."</td><td>".number_format($pledgeTotalsList[$pl->ID], 2, ".", "")."</td>";
            }
    ...
    private static function getGifts($donationId = 0) {
            ...
            $giftsTotal = 0;
            static::$giftsTotalTable = "<table style='width:100%'><tbody><tr style='text-align:left'><th>Gift</th><th>Quantity</th><th>Child</th><th>Amount</th></tr>";
            try {
                ...
                while ($gift = $giftReturn -> next()) {
                    $gifts[] = $gift;
                    static::$giftsTotalTable .= "<tr><td>".$gift->Item->LookupName."</td><td>".$gift->Quantity."</td><td>".$gift->Child->FullName."</td><td>".  number_format(($gift->Quantity * $gift->Item->Amount), 2, ".", "")."</td>";
            ...
            static::$giftsTotalTable .= "<tr style='line-height: 30px;border-top; 1px solid gray'><td colspan=3>Total:</td><td>$".$formattedGiftTotal."</td></tbody></table>";
            return $gifts;
        }
    ...
    public static function logToFile($action, $message){
...
implements RNCPM\ObjectEventHandler_TestHarness {
    ...
    public static function fetchObject($action, $object_type) {
        ...
        // $mailinghelper -> save();
//          
        static::$mailling_helper = RNCPHP\helpers\mailinghelper::fetch(365653);
